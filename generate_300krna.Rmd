...
```{r}
merge_sj <- fread('src/junctions_merged_processed_ensembl.tsv.gz', nThread = 4)
...
```{r}
# get mis-splicing events for ensembl donors
...
ms_donors <- get_missplicing_table_donors(exons_df_donor,merge_sj, c('sample_count_gtex', 'max_uniq_reads', 'sample_count_sra'),
                                          exons)
```

```{r}
# get mis-splicing events for ensembl acceptors
...
ms_acceptors <- get_missplicing_table_acceptors(exons_df_acceptor,merge_sj, c('sample_count_gtex', 'max_uniq_reads', 'sample_count_sra'),
                                                exons)
```

```{r}
# merge mis-splicing events for ensembl transcripts
...
ms_all2 <- fread('output/300KRNA_ensembl.tsv.gz')
```
...
```{r}
# get mis-splicing events for refseq donors
...
ms_donors_refseq <- get_missplicing_table_donors(exons_df_donor_refseq,merge_sj_refseq, 
                                                 c('sample_count_gtex', 'max_uniq_reads', 'sample_count_sra'),
                                                 exons_refseq)
...
```{r}
# get mis-splicing events for refseq acceptors
...
```{r}
# merge mis-splicing events for refseq transcripts
